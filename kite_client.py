...
from kite_service import KiteService, rate_limiter, nse_expiry_calendar
...
class KiteClient:
    ...
    def filter_strikes_wide_range(self, options, spot, itm_depth=5, otm_depth=3):
        """
        Wide strike filtering - Deep ITM to OTM (Production-Grade)
        Strategy: Deep ITM (5) to OTM (3) = 8 strikes total
        ...
        Args:
            options: List of option instruments
            spot: Current spot price
            itm_depth: Number of strikes ITM (default 5)
            otm_depth: Number of strikes OTM (default 3)
    ...
    def get_option_chain_optimized(self, symbol, spot_price):
        ...
        try:
            ...
            # For CALL: ATM - 5 to ATM + 3 (ITM to OTM)
            # For PUT: ATM - 3 to ATM + 5 (ITM to OTM)
            filtered, atm = self.filter_strikes_wide_range(options, spot_price, itm_depth=5, otm_depth=3)
