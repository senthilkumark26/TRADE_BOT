...
# from gemini_integration import GeminiIntegration  # File not found, commented out
from utils.optionstar import generate_trade
# from llm_state_worker import LLMStateWorker  # File not found, commented out
# from trade_queue_system import TradeQueueSystem  # File not found, commented out
# from trade_memory import add_trade_result  # File not found, commented out
...
FROZEN_LLM_COOLDOWN_SECONDS = 3   # REDUCED: 3 seconds between LLM calls (TinyLlama is fast)
FROZEN_LOOP_DELAY_SECONDS = 30     # FROZEN: 30 seconds between scan cycles
FROZEN_MEMORY_THRESHOLD_PERCENT = 90  # INCREASED: Skip LLM if memory > 90% (TinyLlama is light)
FROZEN_MEMORY_CLEANUP_FREQUENCY = 20  # FROZEN: Cleanup every 20 instruments (less frequent)
FROZEN_MODEL_UNLOAD_FREQUENCY = 10     # FROZEN: Unload model every 10 calls (less frequent)
FROZEN_TRADE_STRENGTH_THRESHOLD = 1   # REDUCED: Minimum trade strength 1/5 for LLM (less filtering)
FROZEN_MEMORY_GUARD_THRESHOLD = 85   # INCREASED: Sleep if memory > 85% (TinyLlama is light)
...
class CleanTradingBot:
    ...
    def evaluate_with_llm(self, trade):
        ...
        try:
            ...
            # Use ThreadPoolExecutor with 5-second timeout (TinyLlama is fast)
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(get_analysis)
                try:
                    analysis = future.result(timeout=5)
                except concurrent.futures.TimeoutError:
                    logger.error("LLM call timed out after 5 seconds, using fallback")
                    return 50, "HOLD", "LLM call timed out after 5 seconds, using fallback logic"
...
def main():
    ...
    print("  disable watchdog       - Disable watchdog monitoring (enabled by default)")
    print("=" * 80)
        ...
        try:
            ...
            if user_input == "exit":
                ...
                if bot.watchdog_service:
                    ...
                    print("✗ Watchdog service not available")
