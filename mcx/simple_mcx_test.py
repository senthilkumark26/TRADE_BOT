"""
Simple MCX Dashboard Test - Standalone
"""
...
def simple_mcx_test():
    """Simple MCX dashboard test"""
    logger.info("="*80)
    logger.info("SIMPLE MCX DASHBOARD TEST")
    logger.info("="*80)
    ...
    try:
        ...
        if not contract:
            logger.error("No CRUDEOIL MINI contract found")
            return False
        ...
        quote = kite_client.kite.quote([token])
...
""")
        ...
        logger.info("\nSimple MCX Dashboard Test Completed")
        return True
        
    except Exception as e:
        logger.error(f"Simple MCX Dashboard Test Failed: {e}")
        import traceback
        traceback.print_exc()
        return False

if __name__ == "__main__":
    success = simple_mcx_test()
